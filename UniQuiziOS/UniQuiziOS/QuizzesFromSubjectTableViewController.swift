//
//  QuizzesFromSubjectTableViewController.swift
//  UniQuiziOS
//
...
class QuizzesFromSubjectTableViewController: UITableViewController {

    var quizzes  = [Quiz]()
    var subjectPk : Int?
    
    func getCourses(){
        guard let url = URL(string: Properties.getQuizFromCourseURL(subject : subjectPk!)) else {return}
        print(url)
        ...
        session.dataTask(with: url) { (data, response, error) in
            ...
            if let data = data {
                ...
                do{
                    let newItems = try  JSONDecoder().decode([Quiz].self, from: data)
                    self.quizzes += newItems
                }
                catch{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        destination?.quiz = SimplifiedQuiz(quizzes[indexPath.row])
